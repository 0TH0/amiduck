...
void LadderLottery::CsvLoad(std::string name)
{
    csv = new CsvReader();
    csv->Load(name);
...
void LadderLottery::SetMapType(int type, int x, int z)
{
    map_[x][z] = type;
}

//最短経路を探索
bool LadderLottery::Search(CELL startCell, CELL goalCell)
{
    for (int x = 0; x < STAGE_SIZE_X; x++)
    {
        for (int z = 0; z < STAGE_SIZE_Z; z++)
...
//スタート位置からの各マスのコスト計算
void LadderLottery::OpenNode(CELL cell)
{
    //ノードをOpenにする
...
CELL LadderLottery::GetToGoalCell(int num)
{
    //カウントするためのやつ
    int count = 0;
