...
Text::Text() : hPict_(-1), width_(16), height_(32), fileName_("Image\\char.png"), rowLength_(16),scale_({1,1,1})
{
}
...
void Text::Draw(int x, int y, const char* str)
{
	...
	for (int i = 0; str[i] != '\0'; i++)	//文字列の末尾まで来たら終わり
	{
		...
		//次の位置にずらす
		px += width_ / (float)(Direct3D::screenWidth_ / (scale_.x * 2));
	}
}
...
void Text::SetScale(XMFLOAT3 scale)
{
	scale_ = scale;
}